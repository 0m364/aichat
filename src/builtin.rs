...
mod tests {
    ...
    fn test_safe_path_validation() {
        let cases = vec![
            ("fs_cat", json!({ "path": "../Cargo.toml" })),
            ("fs_ls", json!({ "path": ".." })),
            ("fs_mkdir", json!({ "path": "/tmp/test" })),
            ("fs_write", json!({ "path": "../test.txt", "contents": "test" })),
            ("fs_patch", json!({ "path": "../Cargo.toml", "search": "a", "replace": "b" })),
        ];

        for (name, args) in cases {
            let result = run(name, &args);
            assert!(result.is_err(), "Tool {} should have failed for path traversal", name);
            assert_eq!(result.unwrap_err().to_string(), "Invalid path");
        }
    }
}
